...
- **`middleware.ts`** - Clerk middleware for authentication (configured as `proxy.ts`)
...
### Authentication Flow
- **Clerk Integration**: Full authentication system with Clerk middleware (in `proxy.ts`)
...
### Schema Updates
- Modify `src/db/schema.ts` for changes
...
- [x] Create proxy.ts with `clerkMiddleware()` (completed)
- [x] Wrap layout.tsx with `<ClerkProvider>`
...
- **`/docs/data-mutations.md`**: Server Actions and data mutation guidelines
- **`/docs/auth.md`**: Complete authentication patterns and security requirements
- **`/docs/server-components.md`**: Next.js 16 server component standards
...
- `proxy.ts` is used instead of `middleware.ts` for Clerk middleware in this project
...
## Next.js 16 Server Component Requirements
...
- Authentication MUST use `await auth()` from `@clerk/nextjs/server`
